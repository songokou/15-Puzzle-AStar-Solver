/**
 A* using Heuristic 2, calculating Manhattan displacement
 */
...
public class AStarSolver2 {
	...
	public AStarSolver2(State s){
		stateToSolve = duplicateState(s);
		stateToSolve.setH(findManhattan(s));
		stateToSolve.setCost(findManhattan(s));
...
//		s.addSteps("\n");
	 ...
	 * @return
	 */
	private int findManhattan(State s){
		...
		for (int i = 1; i < 16; i++){
			...
			if(temp < i)
				manhattan +=(i-temp)%4+(i-temp)/4;
			if(temp > i)
				manhattan -=(i-temp)%4+(i-temp)/4;
			if(temp == 0)
				manhattan-=(i-Integer.parseInt(tiles.get(15).getName()))%4 + (i-Integer.parseInt(tiles.get(15).getName()))/4;
		}
		return manhattan;
	}
...
//		copyS.addSteps("Right\n\n");//add to solution
	...
	private void moveDown(State s, int move){
		...
		copyS.addNumMoves(1);
//		System.out.println("DNumMoves: " + copyS.getNumMoves());
...
//		copyS.addSteps("Down\n\n");//add to solution
	...
	private void moveUp(State s, int move){
		...
		copyS.addNumMoves(1);
//		System.out.println("UNumMoves: " + copyS.getNumMoves());
